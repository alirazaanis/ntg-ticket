...
async function main() {
  ...
  const customFieldDepartment = await prisma.customField.findUnique({
    where: { name: 'Department' },
  });
  const customFieldLocation = await prisma.customField.findUnique({
    where: { name: 'Location' },
  });
  const customFieldUrgency = await prisma.customField.findUnique({
    where: { name: 'Urgency Level' },
  });
  const customFieldResolutionTime = await prisma.customField.findUnique({
    where: { name: 'Estimated Resolution Time' },
  });
  const customFieldSatisfaction = await prisma.customField.findUnique({
    where: { name: 'Customer Satisfaction' },
  });

  // Add custom field values to various tickets
  const ticketsForCustomFields = [
    {
      ticketNumber: 'TKT-2024-000001',
      department: 'IT',
  ...
  for (const ticketData of ticketsForCustomFields) {
    const ticket = await prisma.ticket.findUnique({
      where: { ticketNumber: ticketData.ticketNumber },
    });

    if (ticket && customFieldDepartment) {
      await prisma.ticketCustomField.upsert({
        where: {
          ticketId_customFieldId: {
            ticketId: ticket.id,
            customFieldId: customFieldDepartment.id,
          },
        },
        update: {},
        create: {
    ...
    if (ticket && customFieldLocation) {
      await prisma.ticketCustomField.upsert({
        where: {
          ticketId_customFieldId: {
            ticketId: ticket.id,
            customFieldId: customFieldLocation.id,
          },
        },
        update: {},
        create: {
    ...
    if (ticket && customFieldUrgency) {
      await prisma.ticketCustomField.upsert({
        where: {
          ticketId_customFieldId: {
            ticketId: ticket.id,
            customFieldId: customFieldUrgency.id,
          },
        },
        update: {},
        create: {
    ...
    if (ticket && customFieldResolutionTime) {
      await prisma.ticketCustomField.upsert({
        where: {
          ticketId_customFieldId: {
            ticketId: ticket.id,
            customFieldId: customFieldResolutionTime.id,
          },
        },
        update: {},
        create: {
  ...
  // Add customer satisfaction ratings to resolved tickets
  const resolvedTickets = [
    { ticketNumber: 'TKT-2024-000013', satisfaction: '5 - Excellent' },
    { ticketNumber: 'TKT-2024-000014', satisfaction: '4 - Good' },
    { ticketNumber: 'TKT-2024-000015', satisfaction: '5 - Excellent' },
    { ticketNumber: 'TKT-2024-000016', satisfaction: '4 - Good' },
    { ticketNumber: 'TKT-2024-000032', satisfaction: '5 - Excellent' }, // Admin license audit
  ];
  ...
  for (const ticketData of resolvedTickets) {
    ...
    if (ticket && customFieldSatisfaction) {
      await prisma.ticketCustomField.upsert({
        where: {
          ticketId_customFieldId: {
            ticketId: ticket.id,
            customFieldId: customFieldSatisfaction.id,
          },
        },
        update: {},
        create: {
  ...
  console.log('✅ Custom field values added to tickets');
